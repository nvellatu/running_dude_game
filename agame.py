...
class Obstacle(pygame.sprite.Sprite):
    def __init__(self, type):
        ...
        if type == 'fly':
            frame_1 = pygame.image.load('graphics/fly/fly1.png')
            frame_2 = pygame.image.load('graphics/fly/fly2.png')
            y_pos = 210
        else:
...
def collisions(player, obstacles):
    if obstacles:
        for obstable_rect in obstacles:
            if player.colliderect(obstable_rect): return False
    return True
...
title_surf = pygame.transform.rotozoom(title_surf, 0, 1.5)
title_rect = title_surf.get_rect(center = (400,80))

instructions_surf = test_font.render('Press space to start', False, (111,196,169))
instructions_surf = pygame.transform.rotozoom(instructions_surf, 0, 1)
...
while True:
    ...
    if game_active:
        ...
        # # pygame.draw.ellipse(screen, 'Brown', pygame.Rect(50,200,100, 100), 100)
        # screen.blit(score_surf, score_rect)
        score = display_score()
        ...
        if score == 0:screen.blit(instructions_surf, instructions_rect)
            ...
            score_rect = score_msg.get_rect(center = instructions_rect.center)
